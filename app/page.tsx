      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-10 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              {policyCase.agency && (
                <p className="mt-2 text-sm text-gray-600">
                  Agency: {policyCase.agency}
                </p>
              )}

              {policyCase.jurisdiction && (
                <p className="mt-1 text-sm text-gray-600">
                  Jurisdiction: {policyCase.jurisdiction}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">
                {policyCase.sources.length} sources
              </span>

              <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700">
                {policyCase.evidence.length} evidence items
              </span>

              <span className="rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold text-purple-700">
                {policyCase.insights.length} insights
              </span>
            </div>
          </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 space-y-4">
              {policyCase.evidence.map((evidence) => (
                <article
                  key={evidence.id}
